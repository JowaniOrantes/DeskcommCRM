...
atualizar_supabase_single_server() {
  ...
  if [ "$atual" != "$SUPABASE_REF" ]; then
    step "$(t "Atualizando o Supabase desta VPS ({1} → {2})" "${atual:-desconhecida}" "$SUPABASE_REF")"
    if ! (cd "$dir" && env -i PATH="$PATH" HOME="${HOME:-/root}" sh update.sh --to "$SUPABASE_REF" --yes); then
      c_ylw "$(t "⚠ O Supabase não foi atualizado; segue na versão {1}. A próxima atualização tenta de novo." "${atual:-anterior}")"
    fi
  fi
