/**
 * J19.6 — UMA NOVA SUBMISSÃO DO RESPONDI AUTORIZA A IA (e o retorno do lead
 ...
 *   E2E_PORT=3001 pnpm exec playwright test tests/e2e/j19-elegibilidade-respondi.spec.ts
 */
...
test.describe("J19.6 — a submissão do Respondi autoriza a IA", () => {
    ...
    try {
      ...
      const inboundRespondi = await fetch(`${APP_URL}/api/v1/webhooks/waha/${creds.elegibilidade!.waha_path_token}`, {
        ...
        body: JSON.stringify({
          ...
          session,
          payload: {
            id: `false_${respChatId}_J196A${Date.now()}`,
            from: respChatId,
            fromMe: false,
      ...
      const inboundCtrl = await fetch(`${APP_URL}/api/v1/webhooks/waha/${creds.elegibilidade!.waha_path_token}`, {
        ...
        body: JSON.stringify({
          ...
          session,
          payload: {
            id: `false_${ctrlChatId}_J196B${Date.now()}`,
            from: ctrlChatId,
            fromMe: false,
