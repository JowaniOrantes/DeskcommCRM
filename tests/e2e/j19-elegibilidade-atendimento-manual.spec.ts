/**
 * J19.18 — EU RESPONDO O CLIENTE À MÃO PELO WHATSAPP, E A IA PARA.
 *
 ...
 *   E2E_PORT=3001 pnpm exec playwright test tests/e2e/j19-elegibilidade-atendimento-manual.spec.ts
 */
...
const EVIDENCIA = path.join(process.cwd(), ".superpowers/evidence/j19-elegibilidade");
...
test.describe("J19.18 — resposta manual pelo celular pausa a IA (sem apagar a autorização)", () => {
    ...
    try {
      ...
      expect(
        await postWaha({
          ...
          session,
          payload: {
            id: `false_${digits}@c.us_J19MANUAL${Date.now()}`,
            from: chatId,
            fromMe: false,
            body: "Oi, vi o formulário de vocês. Podem me passar os valores?",
            type: "chat",
            timestamp: Math.floor(Date.now() / 1000),
            _data: { notifyName: "Cliente J19.18" },
          },
        }),
      ).toBe(200);
      ...
      expect(
        await postWaha({
          ...
          session,
          payload: {
            id: `true_${digits}@c.us_J19REPLY${Date.now()}`,
            to: chatId,
            fromMe: true,
      ...
      expect(
        await postWaha({
          ...
          session,
          payload: {
            id: `true_${digits}@c.us_J19REPLY2${Date.now()}`,
            to: chatId,
            fromMe: true,
            body: "Segue a tabela.",
            type: "text",
