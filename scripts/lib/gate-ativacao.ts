  ...
  if (semFonte === CAMINHOS_COM_GATE.length) {
    return {
      ...
      detalhe:
        ...
        "(testes J19 em docs/testing/user-journey-map.md). Rode de um checkout para a verificação estática.",
    };
  }
  return {
    status: "PASS",
...
export async function checkRespondiAutoriza(ctx: CtxAtivacao): Promise<Resultado> {
  ...
  linhas.push(
    fonteOk === true
      ...
      : "código não verificável aqui — comportamento coberto por J19.6 no user-journey-map",
  );
