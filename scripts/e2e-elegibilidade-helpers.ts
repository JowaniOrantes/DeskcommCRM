/**
 * Helpers de SQL cru para os E2E do GATE DE ELEGIBILIDADE DA IA (J19):
 *
 *   tests/e2e/j19-elegibilidade-respondi.spec.ts          (J19.6)
 *   tests/e2e/j19-elegibilidade-followup.spec.ts          (J19.12)
 *   tests/e2e/j19-elegibilidade-atendimento-manual.spec.ts (J19.18)
 *
 ...
 *      quando o caso sob teste é outro (J19.12 e J19.18).
 *
...
async function main(): Promise<void> {
  ...
  try {
    switch (cmd) {
      ...
      // faria (J19.18 quer partir de uma conversa JÁ autorizada). Não passa
      ...
      // A linha de `followup_enrollments` (ou `null`). J19.12: o autorizado TEM,
      ...
      case "cleanup-contact": {
        ...
        // Um lead pode ter nascido pelo webhook do Respondi (J19.6). Timeline
      ...
      //   - sem pointerId: `followup` fica desligado (J19.6)
      //   - com pointerId: `followup = {enabled:true, flow_pointer_ids:[id]}` (J19.12)
      //
      // O NOME depende de ter pointerId: as specs J19 rodam em paralelo contra o
      // mesmo banco e o `followup` é coluna compartilhada — se J19.6 e J19.12
      // dividissem a linha, o publish-agent de um sobrescreveria o `followup` do
      // outro (J19.12 quer `enabled:true`; J19.6, `false`). Agentes separados não
      ...
      case "publish-agent": {
        ...
        try {
          ...
          // As specs J19 rodam em paralelo contra o MESMO banco e mais de uma
